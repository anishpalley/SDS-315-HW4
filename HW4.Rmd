...
flags_p_value
```
P-value: The p-value shown above varies every run due to Monte Carlo variance, but it is always below 0.05 which suggests that the observed number of trade flags, 70, is unlikely to have occured by random chance. 
...
violations_p_value
```
P-value: The p-value shown above varies every run due to Monte Carlo variance, but it is always below 0.05 which suggests that the observed number of health code violations, 8, is unlikely to have occured by random chance. 
...
Conclusion: Since the p-value is less than 0.05, we reject the null hypothesis and have sufficient evidence to indicate that the distribution of jurors empaneled by this judge is significantly different from the county’s population proportions and that the judge should be reviewed for systematic bias and peremptory challenges. It does suggest system bias in jury selection for this judge. Other explanations could be that the source of the study was from a biased location and therefore were more partial towards certain groups of people.
...
print(data.frame(Sentence = sentences, p_values = p_vals))


```
The table above shows that the 6th sentence is the most likely to have been produced by an LLM and watermarked to subtly adjust its frequency distribution over letters. The 6th sentence has the smallest p-value and the largest chi-squared value. A higher chi-squared statistic value indicates a greater difference between the observed and expected frequency distributions of the letters. A larger difference between frequency distribution of letters indicates a lower p-value since there is more evidence to suggest that the LLM had produced the sentence.
